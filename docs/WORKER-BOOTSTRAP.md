...
The changes alternate sides (item 1174): a change out of an even era (0 to 1, 2 to 3, ...) starts its ring at the left edge, the computer's point put just behind the player's paddle, and a change out of an odd era (1 to 2, 3 to 4, ...) starts it at the right edge, the player's own point put just past the computer's paddle -- and a check names the edge each ring came from. Both points are placed, not played for (item 1252): the even rungs used to dodge a live rally and hope the computer scored, and on the Nintendo 64's 32 ms frames the player won that point instead on 2 of 6 climbs, starting the ring from the wrong edge.
...
- **A long arrival frame can be the SOUND, not a draw** (item 1264). At 1920x1080 with the GPU
  on, the Xbox 360's arrival had one 113-125 ms frame, its very first, and the trace found no
  ...
  not covered by `warmUp` at that size (`merged-0to1-cut.trace.json.gz`). Item 1285 named it:
  the next bullet.
- **An arrival FLOURISH changes what it draws as its ring grows, so warm it at its ring's own
  moments** (item 1285). The warm-up ran every ring's composite but never a flourish, and era 1's
  television switches its rolled picture, scanlines, colour bleed and glowing band on only once
  the ring is wider than one field unit (0.18 s in), then opens holes in them once the ring is
