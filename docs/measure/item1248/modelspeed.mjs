/*
 * Item 1248 (copied from item 1192's eraspeed.mjs): what do the polygon players cost?
 * One second of ordinary play at eras 5 to 10, on the page's own rAF clock -- the same
 * reading tools/playtest.mjs takes on its climb -- in the playtest's own Chrome
 * (--disable-gpu, display on), under the setups named in SETUPS below.
 *
 * item 1192's setups, for the record:
 *
 ...
 *
 *   node docs/measure/item1248/modelspeed.mjs [--port 9406]
 *
 * Writes modelspeed.json beside itself.
 */
...
async function measure(setup, gpu, display) {
  ...
  try {
    ...
    for (let era = 5; era <= 10; era++) {
      await s.send('Page.navigate', { url: `${base}?era=${era}${display}` });
      for (let t = 0; t < 100; t++) {
...
// setup name -> the query it adds to ?era=N (the third argument, 'display', carries it)
const SETUPS = { sprites: '&models=off', models: '', 'all-lo': '&model=player-proof-lo', 'all-mid': '&model=player-proof-mid' };
const only = process.argv.includes('--setup') ? process.argv[process.argv.indexOf('--setup') + 1].split(',') : Object.keys(SETUPS);
const all = [];
