/*
 * Item 1239: item 1218's recorder (docs/measure/item-1218/trace.mjs), copied so
 * its output lands here, with three changes: the Chrome launch is awaited and
 * guarded (tools/chrome.mjs became async in item 1220, and the copy item 1218
 * left called .close() on a promise); --query <q> adds to the page's address
 * (e.g. --query display=off); and --pre <js> runs one expression on the page
 * after it loads and before the game starts, for an A/B that switches one
 * thing off (e.g. --pre "PongDisplay.row(4).overlay='none'").
 *
...
async function leg(n) {
  ...
  const chrome = await launchChrome(CHROME, ['--headless=new', `--remote-debugging-port=${PORT}`,
    '--mute-audio', '--no-first-run', '--window-size=1000,760', 'about:blank'], { name: 'item1239' }).catch(refusePortTaken);
  let ws;
  try {
    let wsUrl = null;
    ...
    const url = pathToFileURL(path.join(ROOT, 'index.html')).href + '?era=3' + (QUERY ? '&' + QUERY : '');
...
const out = { label: LABEL, query: QUERY, pre: PRE, noFlourish: NO_FLOURISH, atRaw: AT_RAW, noAudio: NO_AUDIO, control: CONTROL, timing: TIMING, fromLoad: FROM_LOAD, root: ROOT, chrome: CHROME, when: new Date().toISOString(), categories: CATEGORIES, results };
