/*
 * Item 1154's browser proof: the change from era 7 to era 8 in the real page,
 ...
 * beat of the PlayStation 2 arrival (the bars sliding in and the colour
 * draining, the boot towers mid-arrival, the flare sweeping home). Writes its PNGs and
 * ps2-shots.json beside this file.
 *
 *   node docs/shots/item-1154/ps2-shots.mjs [--port 9354]
 */
...
const PORT = i > 0 ? Number(process.argv[i + 1]) : 9354;
...
const chrome = await launchChrome(CHROME, ['--headless=new', `--remote-debugging-port=${PORT}`,
  '--mute-audio', '--no-first-run', '--window-size=1000,760', 'about:blank'], { name: 'ps2-arrival' }).catch(refusePortTaken);
...
try {
  ...
  const url = pathToFileURL(path.join(ROOT, 'index.html')).href + '?era=7';
  ...
  const probe = `(() => { const g = window.__pong; const R = window.PongRender; const m = R.eraChangeMoment(g);
    return { era: g.era, raw: m ? m.raw : null, p: m ? m.p : null, wiping: !!(m && m.wiping), cardUp: !!(m && m.cardUp),
      serveDelay: g.serveDelay, paddleY: g.left.y, hook: typeof R.eraLook(8).flourish }; })()`;
  ...
  const force = `(() => { const g = window.__pong; g.era = 7; g.startEra = 0; g.phase = 'playing';
  ...
  await evalJs(`(() => { const g = window.__pong; g.era = 7; g.phase = 'playing'; return 1; })()`);
  await sleep(800);
  await evalJs(force);
  await sleep(250);
  await mouse(0.1);
  ...
  await evalJs(force);
  const shots = [];
  for (const [name, target] of [['ps2-beat1-bars', 0.3], ['ps2-beat2-towers', 0.45], ['ps2-beat3-flare', 0.72]]) {
  ...
  writeFileSync(path.join(HERE, 'ps2-shots.json'), JSON.stringify(out, null, 1) + '\n');
