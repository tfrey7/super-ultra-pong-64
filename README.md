...
alpha on a native-sized copy of the picture, which the page gets with one plain
draw (item 1240) -- no per-pixel work, and one blend over the page's own pixels.
Item 1201 had taken the Genesis's glow off when those blends were still made on
the page (18.2 ms a frame against 16.7 with no screen, in the playtest's
software-drawn Chrome); since item 1240 every 2D era and every ring between two
of them holds 16.7 ms with its screen on, the Genesis's glow included.
`docs/measure/item1201/overlaycost.mjs` times every era with its screen on and
off, and its ring. The playtest's pixel checks read the native picture
